 ...
 * @author Matheus
 */
public class HandlerELogico  extends AbstractHandler{

    public HandlerELogico(Token token) {
        super(token);
    }
    ...
    public void executar(Token token) {
        if (token.getSimbolo().toString().toLowerCase().compareTo("&&") == 0){
            ...
            this.setProximo(new HandlerOuLogico(token));
        }
    }
    ...
    public String recuperarErrosLexico(Token token) {
        if(StringUtils.similarity(token.getSimbolo(), "&&") >= 0.8 ){
            ...
            return "Esse token tem similaridade com: operador_logico_and; "+ proximo.recuperarErrosLexico(token);
        } 
