...
public class HandlerIgual  extends AbstractHandler{
    ...
    public String recuperarErrosLexico(Token token) {
        if(StringUtils.similarity(token.getSimbolo(), "=") >= 0.8 ){
            return "Esse token pode ser substituido por: operador_atribuicao_igual ";
        } else if(StringUtils.similarity(token.getSimbolo(), "=") > 0.5 ){
            return "Esse token tem similaridade com: operador_atribuicao_igual; "+ proximo.recuperarErrosLexico(token);
        } 
