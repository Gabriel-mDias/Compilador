 ...
 * @author Matheus
 */
public class HandlerFechaParentese  extends AbstractHandler{

    public HandlerFechaParentese(Token token) {
        super(token);
    }
    ...
    public String recuperarErrosLexico(Token token) {
        if(StringUtils.similarity(token.getSimbolo(), ")") >= 0.8 ){
            return "Esse token pode ser substituido por: instrucao_fecha_parentese ";
        } else if(StringUtils.similarity(token.getSimbolo(), ")") > 0.5 ){
            return "Esse token tem similaridade com: instrucao_fecha_parentese; "+ proximo.recuperarErrosLexico(token);
        } 
